...
def get_files(root, test=False):
    ...
    path1 = os.path.join('/tmp', data_root1, normalname[1])  # 0->1797rpm ;1->1772rpm;2->1750rpm;3->1730rpm
    data, lab = data_load(path1, axisname=normalname[1],label=0)  # nThe label for normal data is 0

    for i in tqdm(range(len(dataname2))):
        path2 = os.path.join('/tmp', data_root2, dataname2[i])
...
def data_transforms(dataset_type="train", normlize_type="-1-1",snr=0):
    transforms = {
        'train': Compose([
            Reshape(),
...
class CWRU(object):
    ...
    def __init__(self, data_dir,normlizetype,snr):
        self.data_dir = data_dir
        self.normlizetype = normlizetype
        self.snr = snr
        # TODO:修改你要保存数据
        self.save_source = r'F:\资源\学习\深度学习\故障诊断\数据集\学习\dds\CWRU_DATA\CWRU_dataset111HP_4db'  # base-(splited1024) 保存数据的目录，需手动创建
        self.data_preprare()
    def data_preprare(self, test=False):
        ...
        if test:
            ...
            train_pd, val_pd = train_test_split(data_pd, test_size=0.30, random_state=40, stratify=data_pd["label"])
            train_dataset = dataset(list_data=train_pd, transform=data_transforms('train',self.normlizetype,self.snr))
            val_dataset = dataset(list_data=val_pd, transform=data_transforms('val',self.normlizetype,self.snr))
            try:
                os.makedirs(os.path.join(self.save_source))  # 这里做了个替换将mkdir->makedirs,之前老是报错
            except:
                shutil.rmtree(os.path.join(self.save_source))
                os.makedirs(os.path.join(self.save_source))
            torch.save(train_dataset, os.path.join(self.save_source, 'train'))
            torch.save(val_dataset, os.path.join(self.save_source, 'test'))

root='F:\资源\学习\深度学习\故障诊断\数据集\cwru'
normlizetype='1-1'
snr=4
split=CWRU(root,normlizetype,snr)