...
def logistic_regression(data, labels, num_iterations, learning_rate):
...
def stocGradAscent1(dataMatrix, classLabels, numIter=20000):
    m,n = np.shape(dataMatrix)
    weights = np.ones(n)   #initialize to all ones
    for j in range(numIter):
        dataIndex = list(range(m))
        for i in range(m):
            alpha = 4/(1.0+j+i)+0.0005    #apha decreases with iteration, does not
...
def main():
    # 1. 收集及准备数据
    data, labels = load_data('C:\\Users\\hp\\Desktop\\ai\\exp1\\horseColicTraining.txt')
    # 4. 训练算法
    num_iterations = 10000
    learning_rate =0.001
    weights = logistic_regression(data, labels, num_iterations, learning_rate)
    weights = stocGradAscent1(data, labels, numIter=150)
    # 5. 测试算法
    data_test, labels_test = load_data('C:\\Users\\hp\\Desktop\\ai\\exp1\\horseColicTest.txt')
