...
*/	
	...
	TIM_TimeBaseInitStructure.TIM_Period = 20000 - 1;//自动重装器值ARR的值，也就是计数器
	TIM_TimeBaseInitStructure.TIM_Prescaler = 72 - 1;//预分频器值，也就是PSC
	...
	TIM_OC2Init(TIM2, &TIM_OCInitStructure);//这里使用的是PA1，对应第2个输出比较通道，在TIM2上OC2通道，见引脚定义默认复用功能
...
//此函数用于更改通道2的CCR值，见PPT55页，引脚定义表中对应的TIM2_CH2，这个1就是通道2
void PWM_SetCompare2(uint16_t Compare)//云台上面的舵机
{
	TIM_SetCompare2(TIM2, Compare);
}

void PWM_SetCompare3(uint16_t Compare)//云台下面的舵机
{
	TIM_SetCompare3(TIM2, Compare);
}

void PWM_SetCompare2_3(uint16_t Compare)//这个函数用于同时给上下两台舵机赋值相同占空比
{
	TIM_SetCompare2(TIM2, Compare);
	TIM_SetCompare3(TIM2, Compare);
}
