...
int main(void)
{
	...
	while (1)
	{
		...
		if (Serial_GetRxFlag() == 1)	//如果接收到数据包
		{
			...
			OLED_ShowHexNum(4, 4, Serial_RxPacket[1], 2);  	//Serial_RxPacket()将触发USART1的中断
...
//原江科大例程使用的按键是PB1，初始化了PB1和PB11，这里我使用了cw32的拓展版便于方便使用按键PA13,初始化了PB13和PB14
//按下PB13由主控发送数据包到电脑串口助手，串口助手发送数据包到主控，OLED显示发送与接收的数据
