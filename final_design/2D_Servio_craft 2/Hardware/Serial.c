...
//				 Serial_RxFlag=1;//测试-----能进入---------
		 ...
		 Serial_RxFlag=1;//测试-----能进入---------	
		 OLED_ShowNum(4, 10,123 , 3);
		 
		 if (rect_rx_state == 2)//如果到达接收(矩形)数据末状态----------------------------------
		 {
//			 Serial_RxFlag=1;//测试----------不能进入----------
			 
			 if (RxData == '%%')//且收到了矩形的包尾
			 {
				 Serial_RxFlag=1;//测试-----不能进入----------
//				 parse_rect(rx_buffer);
				 OLED_ShowNum(2, 7, 777, 3);
...
uint32_t parse_rect(uint8_t* rx_buffer)//解析矩形的数据包函数
{
    if (rx_buffer == NULL)// 如果缓冲区为空，则退出函数
    {
        return;
