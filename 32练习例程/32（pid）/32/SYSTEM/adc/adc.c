...
void ADC_Configuration(void){ //初始化设置
	...
	ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None;//有软件控制转换 禁止外部触发
	ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;//转换数据右对齐
	ADC_InitStructure.ADC_NbrOfChannel = 1;//!!!顺序进行规则转换的ADC通道的数目（根据ADC采集通道数量修改）
	ADC_Init(ADC_X, &ADC_InitStructure); //根据ADC_InitStruct中指定的参数初始化外设ADCx的寄存器
	//设置指定ADC的规则组通道，设置它们的转化顺序和采样时间
	//ADC1,ADC通道x,规则采样顺序值为y,采样时间为28周期		
  
	ADC_RegularChannelConfig(ADC_X, ADC_Channel_8, 1, ADC_SampleTime_13Cycles5);//!!! ADC1选择通道 x,采样顺序y,采样时间n个周期
	//ADC_RegularChannelConfig(ADC1, ADC_Channel_7, 2, ADC_SampleTime_28Cycles5);//!!! ADC1选择通道 x,采样顺序y,采样时间n个周期
    
    ADC_ITConfig(ADC_X,ADC_IT_EOC,ENABLE);
    
	//ADC_DMACmd(ADC1, ENABLE);// 开启ADC的DMA支持（要实现DMA功能，还需独立配置DMA通道等参数）
	ADC_Cmd(ADC_X, ENABLE);//使能ADC1
	ADC_ResetCalibration(ADC_X); //重置ADC1校准寄存器
	while(ADC_GetResetCalibrationStatus(ADC_X));//等待ADC1校准重置完成
	ADC_StartCalibration(ADC_X);//开始ADC1校准
	while(ADC_GetCalibrationStatus(ADC_X));//等待ADC1校准完成
    
    
	//ADC_SoftwareStartConvCmd(ADC_X, ENABLE); //使能ADC1软件开始转换
}
void ADC_NVIC_CONFIG()
{
	...
	NVIC_INITST.NVIC_IRQChannel=ADC1_2_IRQn;   //中断线
	NVIC_INITST.NVIC_IRQChannelCmd=ENABLE;  //使能中断
	NVIC_INITST.NVIC_IRQChannelPreemptionPriority=1;  //抢占优先级 2
	NVIC_INITST.NVIC_IRQChannelSubPriority=1;     //子优先级  2
